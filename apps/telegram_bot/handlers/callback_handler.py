...
from apps.telegram_bot.services.runtime_config import get_user_config
...
def handle_callback_query(callback_query: dict, tg, db, settings) -> None:
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                if int(t_chat_id) == chat_id:
                    from apps.telegram_bot.services.interaction_manager import (
                        InteractionManager,
                    )
    ...
    if cb_data.startswith("modelpick:"):
        try:
            _, page_str, idx_str = cb_data.split(":", 2)
            page = int(page_str)
            idx = int(idx_str)
            cfg = get_user_config(user_id, db)
            models = fetch_models(cfg, force_refresh=False)
            if idx < 0 or idx >= len(models):
                tg.send_message(chat_id, "❌ 模型索引已失效，请重新打开模型列表")
                return

            model_id = models[idx]
            cmd_str = f"/models page {page}"
            logger.info(f"Callback Model Pick: page={page} idx={idx} model={model_id}")
            ...
            logger.error(f"Model pick resolve failed: {e}", exc_info=True)
            tg.send_message(chat_id, "❌ 处理模型选择失败，请重新打开模型列表")
        return
    ...
    if cb_data.startswith("interact:"):
        ...
        if itype == "key":
            ...
            tg.send_message(
                chat_id, "提示: 请使用命令 `/key set <your-key>`", parse_mode="Markdown"
            )
        elif itype == "endpoint":
            tg.send_message(
                chat_id, "🔗 请使用命令 `/endpoint set <url>`", parse_mode="Markdown"
            )
        elif itype == "model_search":
            tg.send_message(
                chat_id,
                "🔍 请使用命令 `/models <keyword>` (暂未实现搜索过滤，请直接列表翻页)",
                parse_mode="Markdown",
            )
        return
