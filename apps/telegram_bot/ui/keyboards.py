...
def build_models_keyboard(
    models: list, current_model: str, page: int = 1, page_size: int = 8
):
    """构建模型分页键盘"""
    # 1. Pagination Logic
    ...
    row = []
    for idx, m in enumerate(page_items, start=start):
        ...
        row.append({"text": label, "callback_data": f"modelpick:{page}:{idx}"})
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
    ...
    if total_pages > 1:
        if page > 1:
            nav_row.append(
                {"text": "⬅️ 上一页", "callback_data": f"cmd:/models page {page - 1}"}
            )
        else:
        ...
        if page < total_pages:
            nav_row.append(
                {"text": "➡️ 下一页", "callback_data": f"cmd:/models page {page + 1}"}
            )
        else:
...
def build_run_actions_keyboard(
    run_id: str, status: str, interaction_options: Optional[Sequence[str]] = None
):
    """运行控制键盘"""
    ...
    if interaction_options:
        ...
        for opt in interaction_options:
            row_opts.append(
                {"text": opt, "callback_data": f"interact:sel:{run_id}:{opt}"}
            )
            if len(row_opts) >= 2:
    ...
    # Status Row
    row1 = [{"text": "🔄 刷新状态", "callback_data": f"cmd:/status {run_id}"}]
...
def build_help_menu(groups: list):
    ...
    # All / Back
    keyboard.append([{"text": "📚 全部命令", "callback_data": "help:all"}])
...
def build_reuse_options_keyboard(prev_run: dict, goal: str):
    ...
    if status in ["running", "pending", "waiting_input"]:
        ...
        keyboard.append([{"text": "🆕 开启新任务", "callback_data": "reuse:new"}])
    ...
    # 取消按钮
    keyboard.append([{"text": "❌ 取消", "callback_data": "reuse:cancel"}])
