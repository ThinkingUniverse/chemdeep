...
def register_models_commands(registry: CommandRegistry):
    ...
    def cmd_models(payload, ctx):
        ...
        if not msg_id_to_edit:
            ...
            if sent:
                msg_id_to_edit = sent["message_id"]
        ...
        try:
            ...
            if not models:
                if msg_id_to_edit:
                    tg.edit_message(
                        chat_id, msg_id_to_edit, "❌ 未找到可用模型或 API 错误"
                    )
                else:
            ...
            if selected_model:
                if selected_model not in models:
                    tg.send_message(
                        chat_id, "❌ 所选模型已不在当前列表中，请先刷新模型列表"
                    )
                    return
                set_user_config(user_id, "model", selected_model, db)
                cfg = get_user_config(user_id, db)

            # Build Keyboard
            kb = build_models_keyboard(models, cfg.model, page)

            lines = [f"📋 **可用模型列表 ({len(models)})**", f"当前: `{cfg.model}`"]
            if selected_model:
                lines.append(f"✅ 已切换到: `{selected_model}`")
            text = "\n".join(lines)
    ...
    def cmd_model_show(payload, ctx):
        ...
        tg.send_message(
            chat_id, text, reply_markup={"inline_keyboard": btns}, parse_mode="Markdown"
        )
    ...
    def cmd_model_set(payload, ctx):
        ...
        if known_models and model_id not in known_models:
            tg.send_message(
                chat_id, f"⚠️ 模型 `{model_id}` 不在列表中，建议先 `/models refresh`"
            )
        ...
        tg.send_message(
            chat_id, f"✅ 模型已设置为: `{model_id}`", parse_mode="Markdown"
        )
    ...
    def cmd_endpoint_set(payload, ctx):
        ...
        tg.send_message(
            chat_id, f"✅ Base URL 已设置为: `{url}`", parse_mode="Markdown"
        )
