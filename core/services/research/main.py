...
def _resolve_fetch_full_text(fetch_full_text: Optional[bool]) -> bool:
    if fetch_full_text is None:
        return settings.DEFAULT_FETCH_FULL_TEXT
...
class DeepResearcher:
    ...
    def execute_search(
        ...
        fetch_full_text = _resolve_fetch_full_text(fetch_full_text)

        # 0. 如果搜索计划里没有 lanfanshu，补充一个（国内友好检索）
        if hasattr(plan, "search_queries") and isinstance(plan.search_queries, list):
            if not any(
                str(q.get("source", "")).lower() == "lanfanshu"
                for q in plan.search_queries
            ):
                kw = (
                    getattr(plan, "research_object", None)
                    or getattr(plan, "goal", None)
                    or "organic synthesis"
                )

                # [P-LFS] 直接使用简洁关键词，下游 sanitizer 会进一步处理
                kw_lfs = str(kw)
                ...
                lanfanshu_query = {
                    ...
                    "metric_focus": "year-filtered",
                    "priority": 2,
                }

                if min_year is not None:
                    lanfanshu_query["yearLow"] = min_year

                plan.search_queries.append(lanfanshu_query)

        # 1. 执行宽泛搜索
        raw_result = self.executor.execute(
            plan, max_per_source, min_year=min_year, min_score=min_score
        )
        ...
        self.notify(
            f"🎯 筛选完成: 原始 {len(raw_papers)} -> 保留 {len(final_papers)} (丢弃 {ignored_count} 篇低分)\n"
            f"Top 3 推荐:\n"
            + "\n".join(
                [
                    f"  {i + 1}. [{p['screening']['total_score']}] {p.get('title')[:40]}..."
                    for i, p in enumerate(final_papers[:3])
                ]
            )
        )

        if fetch_full_text and final_papers:
            self.notify("🌐 开始尝试获取全文（最多 5 篇，可能会打开浏览器）")
            try:
                final_papers = self._fetch_paper_contents(
                    final_papers, max_papers=min(5, len(final_papers))
                )
                self.notify(f"✅ 全文抓取完成，已处理 {len(final_papers)} 篇论文")
            except Exception as e:
                self.notify(f"⚠️ 全文抓取失败: {e}")
        elif not final_papers:
            logger.info("跳过浏览器全文抓取：筛选后无可用论文")
        else:
            logger.info("跳过浏览器全文抓取：未启用 fetch_full_text 参数")
    ...
    def generate_report(
        self, question: str, objectives: list[str], analyses: list[dict]
    ) -> str:
    ...
    def run_iterative_research(
        ...
        # 1. 目标拆解（formalizer）
        from .formalizer import formalize_problem

        decomposed = formalize_problem(goal)
        # 来自 planner 的搜索查询优先，兼容老流程
        plan_v2 = self.generate_plan(goal)
        current_queries = (
            plan_v2.search_queries if getattr(plan_v2, "search_queries", None) else []
        )

        if not current_queries:
            # 兜底查询构造
            core_kw = decomposed.research_object or goal
            if decomposed.control_variables:
                current_queries = [
                    {
                        "keywords": f"{core_kw} {v}",
                        "source": "openalex",
                    }
                    for v in decomposed.control_variables[:3]
                ]
            else:
                current_queries = [
                    {
        ...
        for iteration in range(max_iterations):
            ...
            for p in new_papers:
                ...
                if doi and doi not in seen_dois:
                    ...
                    unique_new.append(
                        p
                    )  # Keep no-DOI papers? maybe duplicate risk but okay.
            ...
            # 5. 充分性判断 (LLM)
            sufficiency = self.evaluator.check_sufficiency(
                goal, method_clusters, len(all_papers)
            )
        ...
        report_path.write_text(
            final_synthesis.get("synthesis_text", ""), encoding="utf-8"
        )
    ...
    def _fetch_paper_contents(
        ...
        from core.services.fetcher.single_fetch import (
            _navigate_to_paper,
            _handle_potential_cf,
        )
        ...
        if not papers_with_doi:
            logger.info("跳过浏览器全文抓取：候选论文均无 DOI，无法构造 DOI 跳转地址")
            return papers
        ...
        try:
            ...
            if browser_ready:
                ...
                if not success:
                    ...
                    if is_real_browser_running(CDP_PORT):
                        self.notify(
                            f"🌐 Edge 浏览器已在端口 {CDP_PORT} 运行 (延迟检测)"
                        )
                    else:
...
def run_research(question_text: str) -> ResearchState:
    ...
    # 1. Initialize State
    state = ResearchState(question=ResearchQuestion(question=question_text))
