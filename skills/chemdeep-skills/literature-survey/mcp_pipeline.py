...
class ChemDeepPipeline:
    ...
    def run_full_pipeline(
        ...
        # 2. score_papers
        scored = self.client.call_tool(
            "score_papers", {"papers": papers, "min_score": min_score}
        )
        # 3. formalize_research_goal
        problem_spec = self.client.call_tool("formalize_research_goal", {"goal": query})
        # 4. generate_hypotheses
        hypotheses = self.client.call_tool(
            "generate_hypotheses",
            {
        ...
        # 5. extract_evidence
        evidence = self.client.call_tool(
            "extract_evidence", {"papers": scored, "problem_spec": problem_spec}
        )
        # 6. evaluate_hypotheses
        eval_result = self.client.call_tool(
            "evaluate_hypotheses", {"hypotheses": hypotheses, "evidence": evidence}
        )
        # 7. cluster_methods
        method_clusters = self.client.call_tool(
            "cluster_methods", {"evidence": evidence}
        )
        # 8. design_verification_plan
        verification_plan = self.client.call_tool(
